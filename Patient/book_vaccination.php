...
function isEligibleForSecondDose($conn, $patientID) {
    $stmt = $conn->prepare("SELECT MAX(vaccinationslot.Date) as LastVaccinationDate FROM vaccinationrecord INNER JOIN vaccinationslot ON vaccinationrecord.SlotID = vaccinationslot.SlotID WHERE PatientID = ? AND DoseNumber = 1");
    $stmt->bind_param("i", $patientID);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $lastVaccinationDate = new DateTime($row['LastVaccinationDate']);
        $today = new DateTime();
        $interval = $lastVaccinationDate->diff($today);
...
if ($username) {
    ...
    if ($patient = $result->fetch_assoc()) {
        ...
        $eligibleForSecondDose = isEligibleForSecondDose($conn, $patientID);
    }
}
?>
        ...
        <div class="row justify-content-center">
            <div class="col-md-4">
                ...
                <div class="card navigation-card" style="height: 200px;">
                    <div class="card-body">
                        <h5 class="card-title">Book your first dose</h5>
                        <p class="card-text">Book your first dose of vaccination, select time, date, and vaccine</p>
                        <a href="book_vaccination_1.php" class="btn btn-primary">Click Here</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                ...
                <div class="card navigation-card" style="height: 200px;">
                    <div class="card-body">
                        <h5 class="card-title">Book your Second dose</h5>
                        <p class="card-text">Book your second dose of vaccination, select time and date</p>
                        <a href="book_vaccination_2.php" class="btn btn-primary">Click Here</a>
                    </div>
                </div>
            </div>
            <?php endif; ?>
